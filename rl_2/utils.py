...
def compute_token_log_probs(
    ...
    # Check if labels_mask exists, if not create it from labels
    if "labels_mask" in inputs:
        shift_labels_mask = inputs["labels_mask"][..., 1:]  # Shape: [batch_size, seq_len-1]
    else:
        # Fallback: create labels_mask from labels (1 where labels != -100, 0 otherwise)
        shift_labels_mask = (shift_labels != -100).float()
...
def evaluate_on_test_set(
    ...
    generations = inference_engine.generate(
        prompt_token_ids=list(test_dataset["input_ids"]), sampling_params=eval_sampling_params
    )
